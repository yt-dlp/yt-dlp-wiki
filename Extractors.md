...
| `tv`            | Required\*                          | Required  |             | Supported\*  | All video and audio formats in HLS (`m3u8`), https and SABR except for ultralow audio, but only SABR is downloadable (see Notes). | Without account cookies, only DRM formats are returned, although there are [workarounds](https://github.com/yt-dlp/yt-dlp/issues/15583#issuecomment-4580771916). Overdownloading or random A/B experiments may lead to only SABR formats being returned.                                               |
