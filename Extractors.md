...
| `tv`            | Required\*                          | Required  |             | Supported\*  | All video and audio formats in HLS (`m3u8`), https and SABR except for ultralow audio, but only SABR is downloadable (see Notes). | Without cookies (account or [guest](https://github.com/yt-dlp/yt-dlp/issues/15583#issuecomment-4580771916)), only DRM formats are returned. Overdownloading or random A/B experiments may lead to only SABR formats being returned.                                               |
| `tv_downgraded` | Required\*                          | Required                                                |           | Unsupported | All video and audio formats in HLS (`m3u8`) and https except for ultralow audio. | Without account cookies, a "Sign in to confirm you're not a bot" error will appear, although there are [workarounds](https://github.com/yt-dlp/yt-dlp/issues/15583#issuecomment-4580771916). This happens even if other clients are not hit with the bot check.                                 |
