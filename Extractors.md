...
| Client         | [Cookies](#exporting-youtube-cookies) | [JS runtime](https://github.com/yt-dlp/yt-dlp/wiki/EJS) | [PO tokens](https://github.com/yt-dlp/yt-dlp/wiki/PO-Token-Guide) required for | SABR             | Format Availability | Notes |
| -------------- | ------------------------------------- | -------------------------------------------------------- | ------------------------------------------------------------------------------ | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------- |
| `web`           | Supported                           | Required                                                | GVS Subs                                                                             | Only        |  All video and audio formats except ultralow audio.   |                               |
| `web_safari`    | Supported                           | Required                                                | GVS, Subs\*                                                  | Supported              | Pre-merged HLS (`m3u8`) formats only without SABR, all video an audio formats except ultralow audio with SABR and po tokens.    |  HLS (`m3u8`) formats do not require PO tokens. However, these formats are limited to 1080p avc1/H.264 video and AAC audio. All other formats (`sabr`) require PO tokens. Only manually-uploaded, `vtt` subtitles are available for download without PO tokens using `--write-auto-subs` - these subtitles are obtained from the HLS manifest. Auto-translated, auto-generated, and other subtitle formats (e.g. `srv3`) require PO tokens.      |
| `web_embedded`  | Supported                           | Required                                                |                        | Supported   | All video and audio formats in HLS (`m3u8`), https and SABR except for ultralow audio. | Only available for embeddable videos.      |
| `web_music`     | Supported                           | Required                                                | GVS                                                     | Supported              | All video and audio formats in HLS (`m3u8`), https and SABR except for 8k quality and ultralow audio. It does not have AV1 formats. | Only available for videos present in https://music.youtube.com. |
| `web_creator`   | Required                            | Required                                                | GVS                                                     | Supported              | All video and audio formats in HLS (`m3u8`), https and SABR except for ultralow audio. | Can be used to bypass the account age-verification requirement for age-restricted videos, given you pass any account cookies.   |
| `mweb`          | Supported                           | Required                                                | GVS      | Supported\* | All video and audio formats in HLS (`m3u8`), https and SABR. | Overdownloading or random A/B experiments may lead to only SABR formats being returned. Not subject to other clients' throttling when downloading in greater than 10MB chunks; this is useful for quicker downloads when using `--download-sections` or faster seeking within `mpv`'s yt-dlp integration.                                                                                    |
| `tv`            | Required\*                          | Required  |             | Supported\*  | All video and audio formats in HLS (`m3u8`), https and SABR except for ultralow audio, but only SABR is downloadable (see Notes). | Without account cookies, only DRM formats are returned. Note: You can bypass the DRM-only formats _without_ signing in by using guest-session cookies aged for ~1+ days, e.g. `yt-dlp --cookies "guestcookies.txt" "YouTube video URL"` > wait ~1 day > download with `tv` client + aged cookies. Overdownloading or random A/B experiments may lead to only SABR formats being returned.                                               |
| `tv_downgraded` | Required\*                          | Required                                                |           | Unsupported | All video and audio formats in HLS (`m3u8`) and https except for ultralow audio. | Without account cookies, a "Sign in to confirm you're not a bot" error will appear, even if other clients are not hit with the bot check. Note: If other clients are usable, you can bypass the sign-in requirement by using guest-session cookies aged for ~1+ days, e.g. `yt-dlp --cookies "guestcookies.txt" "YouTube video URL"` > wait ~1 day > download with `tv_downgraded` client + aged cookies.        |
| `tv_simply`     | Unsupported                         | Required                                                | GVS                                                     | Supported   | All video and audio formats in HLS (`m3u8`), https and SABR except for ultralow audio.            |         |
| `android`       | Unsupported                         | Unsupported                                             | GVS                                                     | Only        | All video and audio formats. | There is no PO token plugin that allows PO token generation for this client.  |
| `android_vr`    | Unsupported                         | Unsupported                                             |             | Supported        | All video and audio formats in HLS (`m3u8`), https and SABR except for ultralow audio. Known to have poor multi-audio support. | "Made for kids" videos are not available.  |
| `ios`           | Unsupported                         | Unsupported                                             | GVS                                                     | Supported   | All video and audio formats in HLS (`m3u8`), https and SABR except for 8k quality and ultralow audio.    | There is no PO token plugin that allows PO token generation for this client. |
