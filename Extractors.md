...
| `mweb`          | Supported                             | Required                                                | GVS                                                                            | Supported   | Has "low" and "ultralow" qualities for audio. Overdownloading or random A/B experiments may lead SABR-only formats. Not subject to other clients' throttling when downloading in greater than 10MB chunks; this is useful for quicker downloads when using `--download-sections` or faster seeking within `mpv`'s yt-dlp integration.                                                                                                      |
| `tv`            | Required\*                            | Required                                                |                                                                                | Supported\* | Without account cookies, DRM-only formats are returned. Note: You can bypass the DRM-only formats _without_ signing in by using guest-session cookies aged for ~1+ days, e.g. `yt-dlp --cookies "guestcookies.txt" "YouTube video URL"` > wait ~1 day > download with `tv` client + aged cookies. Overdownloading or random A/B experiments may lead SABR-only formats.                                                                    |
