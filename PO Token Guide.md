...
PO Tokens are bound to a content binding, such as the user session (Visitor ID or account Session ID) or the video ID. These days most PO Tokens (such as for web GVS/Player) are bound to the video ID, so a new token is required for each video.

These PO Tokens are only valid for a limited time (at least 12 hours), so it will need to be refreshed periodically. However, some reports suggest that the token may be valid for several months.
