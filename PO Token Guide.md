...
For yt-dlp, you can provide PO Token(s) using the [`po_token` extractor argument](https://github.com/yt-dlp/yt-dlp#youtube), or you can [install a plugin](#po-token-provider-plugins) that hooks into the YouTube extractor to handle fetching a PO Token.
